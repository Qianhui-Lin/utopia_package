...
### 这个函数和fillInteractions_fun_OOP_json一模一样
def fillInteractions_fun_OOP_dict_json(model_json,rate_constant
):
    if not model_json:
    ...
    # Asign loose rates
    elimination_rates = eliminationProcesses_json(system_particle_rate_constant_list, SpeciesList)
...
### 一模一样
def eliminationProcesses_json(system_particle_rate_constant_list, SpeciesList):
...
# 从dict_comp中提取comp信息
def inboxProcess_dict_json(sp1, sp2, sp2_system_particle, surfComp_list, dict_comp):
    ...
    for sp2 in system_particle_rate_constant_list:
        ...
        # Different particle or different river section or compartment
            ...
            if sp1["Pcode"].split("_")[1] == sp2["Pcode"].split("_")[1]:
                sol.append(inboxProcess_dict_json(sp1, sp2, sp2_system_particle, surfComp_list, dict_comp))
