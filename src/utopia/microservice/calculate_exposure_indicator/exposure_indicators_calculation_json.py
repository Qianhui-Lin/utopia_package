...
def Exposure_indicators_calculation_json(model_json,flow,result,system_particle_object_list_merged):
    #### EXPOSURE INDICATORS ####
    ...
    Results_extended_EI = result[
        ~result["Compartment"].isin(comp_outBoundares)
    ]
    ...
    for size in size_list:
        ...
        print(Results_extended_EI.index)
        print(Results_extended_EI.index.dtype)
        mass_sizeFraction = Results_extended_EI[Results_extended_EI["index"].str[0] == size]['mass_g'].sum()
        # mass_sizeFraction = Results_extended_EI.loc[
        #     Results_extended_EI["Size_Fraction_um"] == size, "mass_g"
        # ].sum()
    ...
    for k in flow["tables_outputFlows_mass"]:
        ...
        if k in ["Sediment_Freshwater", "Sediment_Coast"]:
            ...
            flow_rising_mass = []
            flow_rising_number = []
            for p in system_particle_object_list_merged:
